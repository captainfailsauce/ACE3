...
if (_cardiacOutput == 0 || {(_bloodLoss / _cardiacOutput) > BLOOD_LOSS_KNOCK_OUT_THRESHOLD}) then {
...
if (!isPlayer _unit) then {
    ...
    hintSilent format["%1, %2, %3\n%4, %5, %6, %7", round(_bloodVolume * 100) / 100, round(_bloodLoss * 1000) / 1000, round((_bloodLoss / _cardiacOutput) * 100) / 100, round(_heartRate), _bloodPressure, round(_painStatus * 100) / 100];
};
